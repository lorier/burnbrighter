...
// Project paths
var project     = 'burnbrighter'                 // The directory name for your theme; change this at the very least!
...
module.exports = {
  ...
  webfonts: {
    build: { // Copies webfonts from `src` to `build`; does not optimize
      src: src+'**/*(*.eot|*.ttf|*.woff|*.woff2|*.css)'
    , dest: build
    }
  , dist: {
  ...
  scripts: {
    bundles: { // Bundles are defined by a name and an array of chunks (below) to concatenate; warning: this method offers no dependency management!
      core: ['core']
    , pageloader: ['pageloader', 'core', 'fontsmoothie']
    , responsivemenu: ['responsivemenu']
    , stickynav: ['stickynav']
    , fontsmoothie: ['fontsmoothie']
    }
  ...
  , php: {
      src: [src+'**/*.php','!'+src+'**/remove_for_production.php'] // This simply copies PHP files over; both this and the previous task could be combined if you like
    , dest: build
    }
  },
